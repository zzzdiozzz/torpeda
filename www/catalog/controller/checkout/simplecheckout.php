...
class ControllerCheckoutSimpleCheckout extends SimpleController {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            foreach ($modules as $m) {
                ...
                if ($m != 'payment_simple' && file_exists(DIR_APPLICATION . $modulesPath . $m . '.php')) {
                    ...
                    if ($this->simplecheckout->getSettingValue('quickOrder')) {
    ...
    private function order() {
        ...
        if (!$this->simplecheckout->getSettingValue('quickOrder') && isset($this->session->data['payment_method'])) {
            $payment_method = $this->session->data['payment_method'];
        }
    ...
    private function confirm() {
        ...
        if ($this->simplecheckout->getOpenCartVersion() < 200) { 
            $this->model_checkout_order->confirm($order_id, $this->simplecheckout->getSettingValue('quickOrderStatus'), '', false);
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->simplecheckout->getSettingValue('quickOrderStatus'), '', false);
        }
    }
}
